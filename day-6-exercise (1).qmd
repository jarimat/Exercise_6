...
arrange(filtered_data, desc(cases)) %>%
  slice_head(n = 5)
```
...
```{r}
five_worst_counties <- covid %>%
  filter(date == most_recent_date) %>%
  filter(cases > 0 & county != "Unknown") %>%
  mutate(percentage_deaths = 100 * (deaths / cases)) %>%
...
```{r}
five_worst_states <- covid %>%
  filter(date == most_recent_date) %>%
  filter(cases > 0 & county != "Unknown") %>%
  group_by(state) %>%
  summarize(total_cases = sum(cases), total_deaths = sum(deaths)) %>%
  mutate(percentage_deaths = 100 * (total_deaths / total_cases)) %>%
  arrange(desc(percentage_deaths)) %>%
  slice_head(n = 5) %>%
